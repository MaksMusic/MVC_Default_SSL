...
mkdir -p "$DEPLOY_DIR/nginx/ssl" "$DEPLOY_DIR/letsencrypt"
...
if [ ! -f "$DEPLOY_DIR/nginx/ssl/cert.pem" ] || [ ! -f "$DEPLOY_DIR/nginx/ssl/key.pem" ]; then
  ...
  if [ -f "$DEPLOY_DIR/nginx/nginx.conf" ]; then
    sed -i "s/java-planet.ru/${DOMAIN}/g" "$DEPLOY_DIR/nginx/nginx.conf"
    sed -i "s/www\.java-planet\.ru/${WWW_DOMAIN}/g" "$DEPLOY_DIR/nginx/nginx.conf"
  fi
